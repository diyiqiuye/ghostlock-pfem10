 ...
 *   [E] = 外部独立实测 : JoinChang/ghostlock-oneplus src/devices/cph2521/offsets.h
 *         (OPPO Reno10 Pro+ CPH2521, SM8475, 同为 5.10.236-android12-9-o, 同 GKI 家族)
 *         2026-09-16 逐项 diff 结论 (docs/64 §2): STRUCT_OFFSETS_5_10 25/26 相同;
 *         13 个全局符号镜像偏移完全相同 (init_cred/selinux_state/kptr_restrict/
 *         empty_zero_page/slide_*); 差异项全部可解释为源树不同导致的段内位移.
 *         ★ 唯一结构体分歧: 上游 fops_splice_read=0xC8 系推导值 (commit 4a67059
 *           自述 "Using 6.12 hardcoded values causes kernel panic"), 我方 [C] 0xC0 为准.
 *         ★ .cfi_jt 是 5.10 厂商 kCFI 的硬要求 (上游 e0fb531: 裸地址 step=33 被拒),
 *           本表全部使用 *_CFI_JT ✓ 移植时勿删.
 *         ★ kimage_text_base 必须运行时可配, 5.10=0xffffffc008000000 (上游 e0fb531/934bc4a).
 *
 ...
 *
 * ★ 2026-09-16 补: 与上游 CPH2521 (SM8475, 同 5.10.236-android12-9-o) 表逐项 diff
 *   (docs/64 §2, tools/diff_cph2521_pfem10.py):
 *   - STRUCT_OFFSETS_5_10 **25/26 相同**, 仅 fops_splice_read 分歧 (我方 0xC0 为准);
 *   - 13 个全局符号镜像偏移**完全相同**, 含全部功能性关键项
 *     (init_cred / selinux_state / kptr_restrict / empty_zero_page / slide_nfulnl_logger /
 *      slide_boot_id / system_unbound_wq / call_umh_exec_work / ashmem_misc_fops);
 *   - 差异项 (selinux_blob_sizes -0x40, ashmem_* -0x1A8, configfs +0x808 等) 全部
 *     可解释为源树不同导致的段内位移 ⇒ **结构体字段可移, 符号不可移**.
 * ============================================================================
 */
...
#define PFEM10_KERNEL_PHYS_LOAD   0xa8000000ULL          /* [X]+[E] OPLUS/SM8450-8650 家族值;
                                                          * [E] 上游 commit 013de28 对 SM8650 vivo 明确设为
                                                          * 0xa8000000; issue #31 SM8475 /proc/iomem 确认。
                                                          * 仍需本机 /proc/iomem 终验 (docs/64 §3) */
...
/* ★ KASLR 泄漏源 X（2026-09-14 离线六条件定案, docs/18 §4.5 + findings.md）:
 * X = sun4i_a10_sram_a3_a4+0x10 (.variant 指针槽)。sunxi_sram 全志驱动在本机
 * 永不 bind (DT 无 allwinner 节点) → 其 .data 运行时=链接期, 破坏面零消费。
 * *(X) = &.compoundliteral.35062 (struct sunxi_sramc_variant, .data 可写 ✓②),
 * *(X+0x08) = 死结构内静态 NULL 字段 ✓④(运行时), X−0x08 = emac_offset 死数值。 */
#define PFEM10_OFF_LEAK_X              0x028D53C8ULL  /* [D] 泄漏源槽 (image off) */
#define PFEM10_OFF_LEAK_V              0x028D5508ULL  /* [D] = *X (link): slide = leaked - (KIMAGE+此值) */
#define PFEM10_OFF_LEAK_MB             0x02B99B70ULL  /* [D] boot_id+0x3: 8 对齐 mailbox 槽 (uuid bytes[3:11)) */
...
/* [C] 编译导出, 并由 CPH2521 同 GKI 家族独立验证 ([E], docs/64 §2.1, 25/26 相同).
 * ⚠ fops_splice_read = 0xC0 是本表最需守住的一项: 上游 CPH2521 表写 0xC8,
 *   但那是从 6.x 布局推导的值 (上游 commit 4a67059 自述 6.12 硬编码值会导致
 *   "VFS reads NULL from the wrong fops slot" panic)。我方 0xC0 为 [C] 编译导出:
 *   5.10 struct file_operations 无 fop_flags (6.12 才有) → copy_file_range 起 0xC0,
 *   splice_read 紧随其后. A 路线 fake fops 表务必用 0xC0, 错槽 = VFS 读 NULL = panic. */
